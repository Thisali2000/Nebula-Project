...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function renderSpecialApprovalTable(students) {
        ...
        students.forEach((student, index) => {
            ...
            if (student.document_url) {
                const fileName = student.document_path ? student.document_path.split('/').pop() : 'Document';
                documentHtml = `<a href="${student.document_url}" target="_blank" class="btn btn-outline-primary btn-sm">
    ...
    function generateCourseRegistrationId(courseId) {
        fetch('/get-next-course-registration-id', {
            method: 'POST',
            headers: {
        ...
        .then(data => {
            if (data.success) {
                document.getElementById('inlineCourseRegId').value = data.course_registration_id;
            } else {
                // Fallback registration ID
                document.getElementById('inlineCourseRegId').value = '2025/HND/SE/004';
            }
        })
        .catch(error => {
